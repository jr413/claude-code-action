...
export class ApiKeyManager {
  ...
  constructor(storePath: string = "/tmp/api-keys.json", encryptionKey?: string) {
    this.storePath = storePath;
    this.encryptionKey = encryptionKey || process.env.API_KEY_ENCRYPTION_KEY || this.generateEncryptionKey();
  }
   ...
   */
  async listApiKeys(userId: string): Promise<Array<Omit<ApiKeyEntry, 'encryptedKey' | 'salt' | 'iv'>>> {
    const store = this.loadStore();
  ...
  async getStats(): Promise<{
    ...
    for (const entry of Object.values(store.keys)) {
      ...
      stats.providerStats[entry.provider] = (stats.providerStats[entry.provider] || 0) + 1;
    }
