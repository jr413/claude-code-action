...
- 身内4人だけで使う想定のため、Supabase の匿名キーで誰でも読み書きできる設定になっている（認証なし）。不特定多数に公開する場合はメンバー認証の追加が必要。
